...
本地小说分段成片工具：输入分段文本，选择按文件名排序的图片文件夹，自动生成带旁白和字幕的视频。
...
在项目根目录创建或编辑 `.env`。`.env` 已被 `.gitignore` 忽略，不要把真实密钥提交出去。
...
1. 在文本框中输入小说文本，每个回车段落会生成一个视频片段。
2. 选择图片文件夹，图片会按文件名自然排序。
3. 确保图片数量不少于文本段落数量。
4. 选择旁白引擎、视频比例、图片动效、字幕等参数。
5. 点击“生成视频”，等待进度条完成。
6. 点击“下载视频”保存成品。
